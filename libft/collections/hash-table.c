...
static int hash_table_allocate_table(HashTable *hash_table)
{
	...
	hash_table->table_size = hash_table->prime_index < hash_table_num_primes ?
					 hash_table_primes[hash_table->prime_index] :
					 hash_table->entries * 10;
	return (!!(hash_table->table = ft_tmemalloc(sizeof(HashTableEntry *), hash_table->table_size)));
}
                          ...
                          HashTableEqualFunc equal_func)
{
	...
	if (!(hash_table = (HashTable *) malloc(sizeof(HashTable))))
		return (NULL);
...
static int hash_table_enlarge(HashTable *hash_table)
{
	...
	while (i < old_table_size)
	{
		while (old_table[i])
		{
			...
			/* Fetch old_table[i] HashTablePair */
			pair = &(old_table[i]->pair);
			/* Find the index into the new table */
			index = hash_table->hash_func(pair->key) % hash_table->table_size;
			/* Link this entry into the chain */
			old_table[i]->next = hash_table->table[index];
			hash_table->table[index] = old_table[i];
                      ...
                      HashTableValue value)
{
	...
	while (rover)
	{
		...
		pair = &(rover->pair);
		if (hash_table->equal_func(pair->key, key)) {
