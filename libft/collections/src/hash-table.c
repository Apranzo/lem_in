...
void hash_map_iterate(t_hash_map *hash_map, t_hm_iterator *iterator)
{
	size_t chain;
	...
	while (chain < hash_map->table_size)
	{
		...
		{
			iterator->next_entry = hash_map->table[chain];
...
{
	t_hm_entry *current_entry;
	t_hash_map *hash_map;
	t_hm_pair pair = {NULL, NULL};
	size_t chain;
